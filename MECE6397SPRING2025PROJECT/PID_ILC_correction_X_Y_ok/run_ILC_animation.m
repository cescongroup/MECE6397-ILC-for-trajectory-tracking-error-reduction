...
% % Create a VideoWriter object for MP4 format
% video2D = VideoWriter('2D_animation_xy_ILC_iter.mp4'); % Output file name
% video2D.FrameRate = 2; % Set frame rate
% open(video2D); % Open the video writer
% video3D = VideoWriter('3D_animation_xy_ILC_iter.mp4'); % Output file name
% video3D.FrameRate = 2; % Set frame rate
% open(video3D); % Open the video writer
...
for j=1:17
    ...
    % frame2D = getframe(gcf); % Get the current figure frame
    % writeVideo(video2D, frame2D); % Write the frame to the video
    ...
    hold off
    set(gca, 'Color', 'white');
    frame3D = getframe(gcf); % Get the current figure frame
    % writeVideo(video3D, frame3D); % Write the frame to the video
    % pause(10);
...
% Create a VideoWriter object for MP4 format
video = VideoWriter('3D_animation_xy_corrected'); % Output file name
